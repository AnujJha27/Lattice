...
import { PORTRAIT_PALETTES, type PortraitPalette, type PortraitTheme } from "@/lib/portraitThemes";
...
function Region({ point, label, score, tone, snapshotId, elementId, onSelect, palette, mobileSecondary = false, children }: {
  point: Point;
  label: string;
  score: number;
  ...
  const color = palette[tone === "emerging" ? "emerging" : tone];
  ...
  return <g role="button" tabIndex={0} aria-label={`${label}, ${tone}, signal ${Math.round(score * 100)} percent`} onMouseEnter={() => trackPortraitEvent("portrait_element_hovered", snapshotId, elementId)} onClick={onSelect} onKeyDown={(event) => activate(event, onSelect)} className={`group cursor-pointer outline-none${mobileSecondary ? " portrait-mobile-secondary" : ""}`} opacity={tone === "dormant" ? 0.5 : 1}>
    ...
    <circle cx={point.x} cy={point.y} r={radius + 13} fill="none" stroke={palette.rule} strokeWidth="2" className="pointer-events-none opacity-0 transition-opacity group-focus-visible:opacity-100" />
    {children}
  </g>;
}
...
export function PortraitRenderer({ portrait, onSelect, theme = "Editorial" }: { portrait: PortraitModel; onSelect: (selection: PortraitSelection) => void; theme?: PortraitTheme }) {
  ...
  return <div className="relative overflow-hidden rounded-2xl border border-[var(--border-strong)] bg-[var(--bg-surface)] shadow-[var(--shadow-lg)]" data-portrait-theme={theme.toLowerCase()} aria-label="Interactive intellectual portrait">
    <svg viewBox="0 0 1000 760" className="block h-auto w-full" role="group" aria-labelledby="portrait-title portrait-description">
      ...
      <desc id="portrait-description">{portrait.portrait_photo_enabled ? "Your explicitly enabled profile photo" : "An anonymous human silhouette"} surrounded by concepts from your Lattice Brain. Concepts inside the human form are anchors; crossing lines are bridges; outer regions are frontiers and threads.</desc>
      <defs>
        <radialGradient id="portrait-sky" cx="50%" cy="42%" r="68%"><stop offset="0" stopColor={palette.skyStart} /><stop offset="0.72" stopColor="#101624" /><stop offset="1" stopColor={palette.skyEnd} /></radialGradient>
        <linearGradient id="portrait-figure" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor={palette.rule} stopOpacity="0.14" /><stop offset="0.52" stopColor={palette.figure} stopOpacity="0.18" /><stop offset="1" stopColor={palette.frontier} stopOpacity="0.12" /></linearGradient>
      ...
      <path d="M120 440C270 110 730 110 880 440" fill="none" stroke={palette.bridge} strokeOpacity="0.09" strokeDasharray="2 14" />
      <path d="M180 595C310 210 690 210 820 595" fill="none" stroke={palette.frontier} strokeOpacity="0.08" strokeDasharray="1 12" />
      <circle cx={PORTRAIT_CENTER.x} cy={PORTRAIT_CENTER.y + 80} r="250" fill={palette.figure} fillOpacity="0.08" filter="url(#portrait-soft-glow)" />
      {emerging.map((thread, index) => { const point = orbitPoint(thread.id, index, emerging.length, 310); return <ellipse key={`emerging-orbit-${thread.id}`} className={index >= 3 ? "portrait-mobile-secondary" : undefined} cx={PORTRAIT_CENTER.x} cy={PORTRAIT_CENTER.y} rx={Math.abs(point.x - PORTRAIT_CENTER.x) + 38} ry={Math.abs(point.y - PORTRAIT_CENTER.y) + 26} fill="none" stroke={palette.emerging} strokeOpacity="0.18" strokeDasharray="5 11" />; })}
      <path d="M500 72c-53 0-86 38-86 89 0 43 21 72 47 88-16 22-42 34-81 53-56 28-84 79-99 143l-31 247h500l-31-247c-15-64-43-115-99-143-39-19-65-31-81-53 26-16 47-45 47-88 0-51-33-89-86-89Z" fill="url(#portrait-figure)" stroke={palette.rule} strokeOpacity="0.26" strokeWidth="2" />
      {portrait.portrait_photo_enabled && <ProfilePhoto />}
      <g clipPath="url(#figure-mask)"><path d="M338 650C410 560 417 420 500 340c83 80 90 220 162 310" fill="none" stroke={palette.figure} strokeOpacity="0.13" strokeWidth="36" /><path d="M420 180c40 28 120 28 160 0" fill="none" stroke={palette.rule} strokeOpacity="0.15" strokeWidth="2" /></g>
      <AnimatePresence initial={false} mode="sync">
        {bridges.map((item, index) => { const start = insidePoint(item.id, index); const end = orbitPoint(item.id, index, bridges.length, 330); return <RegionTransition key={`bridge-line-${item.id}`} origin={start} reducedMotion={reducedMotion} className={index >= 3 ? "portrait-mobile-secondary" : undefined}><path d={`M${start.x} ${start.y} C${start.x - 70} ${start.y - 60}, ${end.x + 70} ${end.y + 60}, ${end.x} ${end.y}`} fill="none" stroke={palette.bridge} strokeOpacity="0.4" strokeWidth="2" strokeDasharray="7 7" /><Region point={start} label={item.name} score={item.score} tone="bridge" palette={palette} snapshotId={portrait.snapshot_id} elementId={item.id} onSelect={() => select("bridge", item)} mobileSecondary={index >= 3} /></RegionTransition>; })}
        {anchors.map((item, index) => <RegionTransition key={`anchor-${item.id}`} origin={insidePoint(item.id, index)} reducedMotion={reducedMotion}><Region point={insidePoint(item.id, index)} label={item.name} score={item.score} tone="anchor" palette={palette} snapshotId={portrait.snapshot_id} elementId={item.id} onSelect={() => select("anchor", item)} mobileSecondary={index >= 4} /></RegionTransition>)}
        {frontiers.map((item, index) => { const point = orbitPoint(item.id, index, frontiers.length, 305); return <RegionTransition key={`frontier-${item.id}`} origin={point} reducedMotion={reducedMotion} className={index >= 4 ? "portrait-mobile-secondary" : undefined}><path d={`M${PORTRAIT_CENTER.x} ${PORTRAIT_CENTER.y + 8} L${point.x} ${point.y}`} stroke={palette.frontier} strokeOpacity="0.12" strokeDasharray="2 9" /><Region point={point} label={item.name} score={item.score} tone="frontier" palette={palette} snapshotId={portrait.snapshot_id} elementId={item.id} onSelect={() => select("frontier", item)} mobileSecondary={index >= 4} /></RegionTransition>; })}
        {emerging.map((item, index) => <RegionTransition key={`emerging-${item.id}`} origin={orbitPoint(item.id, index, emerging.length, 390)} reducedMotion={reducedMotion}><Region point={orbitPoint(item.id, index, emerging.length, 390)} label={item.name} score={item.score} tone="emerging" palette={palette} snapshotId={portrait.snapshot_id} elementId={item.id} onSelect={() => select("emerging_thread", item)} mobileSecondary={index >= 3} /></RegionTransition>)}
        {dormant.map((item, index) => <RegionTransition key={`dormant-${item.id}`} origin={orbitPoint(`${item.id}-dormant`, index, dormant.length, 360)} reducedMotion={reducedMotion}><Region point={orbitPoint(`${item.id}-dormant`, index, dormant.length, 360)} label={item.name} score={item.score} tone="dormant" palette={palette} snapshotId={portrait.snapshot_id} elementId={item.id} onSelect={() => select("dormant_thread", item)} mobileSecondary={index >= 2} /></RegionTransition>)}
        {visuals.map((source, index) => <VisualRegion key={`visual-${source.asset_id}`} source={source} point={orbitPoint(source.asset_id, index, visuals.length, 250)} snapshotId={portrait.snapshot_id} onSelect={() => select("visual", source)} mobileSecondary={index >= 4} reducedMotion={reducedMotion} palette={palette} />)}
      </AnimatePresence>
      <text className="portrait-caption" x="500" y="728" textAnchor="middle" fill="#8b93a7" fontFamily="var(--font-plex-mono)" fontSize="11" letterSpacing="3">{portrait.portrait_photo_enabled ? "OPT-IN PROFILE PHOTO · DATA-BOUND COMPOSITION" : "ANONYMOUS FORM · DATA-BOUND COMPOSITION"}</text>
    </svg>
...
function ProfilePhoto() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(`${PUBLIC_CONFIG.apiUrl}/api/users/me/portrait-photo`, {
        ...
        // The anonymous silhouette remains visible when the private photo cannot load.
      }
    };
    void load();
    return () => {
  ...
  return imageUrl ? <image href={imageUrl} x="250" y="72" width="500" height="620" preserveAspectRatio="xMidYMid slice" clipPath="url(#figure-mask)" opacity="0.78" aria-label="Your opted-in profile photo" /> : null;
}

function VisualRegion({ source, point, snapshotId, onSelect, mobileSecondary = false, reducedMotion, palette }: { source: PortraitVisualSource; point: Point; snapshotId: string; onSelect: () => void; mobileSecondary?: boolean; reducedMotion: boolean; palette: PortraitPalette }) {
  const size = 42;
