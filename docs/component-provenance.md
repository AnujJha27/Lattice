...
Per the build spec (§3): every significant reused or adapted component is
recorded here. Custom product-specific code (Brain interactions, pathway
behavior, grounded-lesson UI) is built on these primitives.
...
| Supabase JS + SSR helpers | supabase/supabase-js · supabase/ssr | MIT | 2.x / 0.6.x | Auth session management, cookie refresh | Standard `@supabase/ssr` cookie pattern per upstream docs |
...
| google-genai | googleapis/python-genai | Apache-2.0 | latest | GeminiProvider + embeddings behind our abstractions | Wrapped by `LLMProvider`/`EmbeddingProvider` protocols |
...
Ranking, classification, dedup, chunking and extraction are Lattice-original
implementations encoding the trust-hierarchy requirements (overrides §8–16).

## Pending phases (planned, not yet integrated)

| Component | Candidate upstream | Intended use |
| --- | --- | --- |
| Brain graph rendering | Sigma.js + Graphology (jacomyal/sigma.js, graphology/graphology) | WebGL rendering of thousands of nodes; camera animation; neighborhood emphasis (spec §7–8) |
...
| Animation | motiondivision/motion | Springs, layout transitions (spec §2) |
